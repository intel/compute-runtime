...
TEST_F(BufferTailPatternTests, givenPlainBufferWhenCheckingTailPatternThenTailIsReportedValidAndReadAtZeroOffset) {
    struct TestCase {
    ...
    const TestCase testCases[] = {
        {"tail fill disabled", 1, 0, 2 * MemoryConstants::pageSize},
        {"size extension disabled", 0, 1, 2 * MemoryConstants::pageSize},
        {"buffer not larger than the tail", 2, 1, MemoryConstants::pageSize},
    };
    ...
    for (const auto &testCase : testCases) {
        ...
        EXPECT_EQ(0u, this->spyQueue->lastOffset) << testCase.description;
        EXPECT_EQ(0u, this->spyQueue->lastSize) << testCase.description;
    }
}
...
TEST_F(BufferTailPatternTests, givenIntactTailPatternWhenCheckingTailPatternThenTailIsReadAtZeroOffsetAndNothingIsPrinted) {
    debugManager.flags.ForceExtendedBufferSize.set(1);
    debugManager.flags.FillBufferTailWithPattern.set(1);
    ...
    EXPECT_EQ(0u, this->spyQueue->lastOffset);
    EXPECT_EQ(0u, this->spyQueue->lastSize);
}

TEST_F(BufferTailPatternTests, givenCorruptedTailPatternWhenCheckingTailPatternThenTailIsReportedValidAndNothingIsPrinted) {
    debugManager.flags.ForceExtendedBufferSize.set(1);
    debugManager.flags.FillBufferTailWithPattern.set(1);
...
TEST_F(BufferTailPatternTests, givenRetainedBufferWithCorruptedTailWhenReleasingMemObjectThenTailIsValidatedOnlyOnTheFinalRelease) {
    ...
    EXPECT_EQ(0u, this->spyQueue->lastOffset);
    EXPECT_EQ(0u, this->spyQueue->lastSize);
    EXPECT_FALSE(hasSubstr(outputAfterFinalRelease, std::string("Buffer tail pattern mismatch"))) << outputAfterFinalRelease;
    EXPECT_FALSE(hasSubstr(outputAfterFinalRelease, std::string("Buffer tail pattern corrupted on release"))) << outputAfterFinalRelease;
}
