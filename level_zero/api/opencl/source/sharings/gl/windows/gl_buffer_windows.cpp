...
GraphicsAllocation *GlBuffer::createGraphicsAllocation(Context *context, unsigned int bufferId, _tagCLGLBufferInfo &bufferInfo) {
    ...
    if (!graphicsAllocation) {
        AllocationProperties properties = {context->getClDevice()->getRootDeviceIndex(),
                                           ...
                                           context->getClDevice()->getDevice().getDeviceBitfield()};
        // couldn't find allocation for reuse - create new
        MemoryManager::OsHandleData osHandleData{bufferInfo.globalShareHandle};
        graphicsAllocation =
            context->getClDevice()->getDevice().getMemoryManager()->createGraphicsAllocationFromSharedHandle(osHandleData, properties, true, false, true, nullptr);
    }
