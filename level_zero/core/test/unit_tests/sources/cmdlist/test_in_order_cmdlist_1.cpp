...
HWTEST_F(InOrderCmdListTests, givenCounterBasedEventsWhenHostWaitsAreCalledThenLatestWaitIsRecorded) {
    ...
    *inOrderExecInfo->getBaseHostAddress() = 2u;
    ...
    auto counterValue = events[1]->inOrderExecHelper.getEventData()->counterValue;
    EXPECT_TRUE(inOrderExecInfo->isCounterAlreadyDone(counterValue, 0));
    EXPECT_TRUE(inOrderExecInfo->isCounterAlreadyDone(events[0]->inOrderExecHelper.getEventData()->counterValue, 0));
...
HWCMDTEST_F(IGFX_XE_HP_CORE, InOrderCmdListTests, givenCounterBasedTimestampEventWhenQueryingTimestampThenEnsureItsCompletion) {
    ...
    event1->getInOrderExecEventHelper().setLastWaitedCounterValue(2, 0);
    event2->getInOrderExecEventHelper().setLastWaitedCounterValue(2, 0);
    event3->getInOrderExecEventHelper().setLastWaitedCounterValue(3, 0);
...
HWCMDTEST_F(IGFX_XE_HP_CORE, InOrderCmdListTests, givenUserInterruptEventWhenWaitingThenPassCorrectAllocation) {
    ...
    EXPECT_EQ(1u, ultCsr->waitUserFenceParams.callCount);
    EXPECT_EQ(events[0]->getInOrderExecEventHelper().getDeviceCounterAllocation(), ultCsr->waitUserFenceParams.latestAllocForInterruptWait);
    EXPECT_TRUE(ultCsr->waitUserFenceParams.userInterrupt);
    ...
    EXPECT_EQ(2u, ultCsr->waitUserFenceParams.callCount);
    EXPECT_EQ(events[1]->getInOrderExecEventHelper().getHostCounterAllocation(), ultCsr->waitUserFenceParams.latestAllocForInterruptWait);
    EXPECT_TRUE(ultCsr->waitUserFenceParams.userInterrupt);
    ...
    *hostAddress = 0;
    ...
    EXPECT_EQ(event2->getInOrderExecEventHelper().getHostCounterAllocation(), ultCsr->waitUserFenceParams.latestAllocForInterruptWait);
    EXPECT_TRUE(ultCsr->waitUserFenceParams.userInterrupt);
...
HWTEST_F(InOrderCmdListTests, givenImplicitEventConvertionEnabledWhenCallingAppendThenHandleInOrderExecInfo) {
    ...
    auto inOrderExecBaseSignalValue = events[0]->getInOrderExecBaseSignalValue();
    auto inOrderExecDataAssigned = events[0]->getInOrderExecEventHelper().isDataAssigned();
    if (inOrderExecDataAssigned) {
        EXPECT_GT(inOrderExecBaseSignalValue, 0u);
    } else {
        EXPECT_EQ(0u, inOrderExecBaseSignalValue);
    }
    immCmdList->appendEventReset(events[0]->toHandle());
    EXPECT_EQ(0u, events[0]->getInOrderExecBaseSignalValue());
    EXPECT_FALSE(events[0]->getInOrderExecEventHelper().isDataAssigned());
...
HWCMDTEST_F(IGFX_XE_HP_CORE, InOrderCmdListTests, givenImmediateCmdListWhenDispatchingWithRegularEventThenSwitchToCounterBased) {
    ...
    if (events[0]->inOrderExecHelper.hasTimestampNodes()) {
        copyOnlyCmdList->inOrderExecInfo->pushTempTimestampNode(events[0]->inOrderExecHelper.getTimestampNode(0), events[0]->inOrderExecHelper.getEventData()->counterValue, 0);
    }
    static_cast<WhiteboxInOrderExecEventHelper &>(events[0]->inOrderExecHelper).timestampNodes.clear();
    events[0]->makeCounterBasedInitiallyDisabled(eventPool->getAllocation());
...
HWTEST2_F(InOrderCmdListTests, givenRelaxedOrderingEnabledWhenSignalEventCalledThenPassStallingCmdsInfo, IsXeHpcCore) {
...
HWTEST2_F(InOrderCmdListTests, givenCounterHeuristicForRelaxedOrderingEnabledWhenSmallTaskIsFlushedThenIncrementCounter, IsXeHpcCore) {
    debugManager.flags.DirectSubmissionRelaxedOrdering.set(1);
...
HWTEST2_F(InOrderCmdListTests, givenCounterHeuristicForRelaxedOrderingEnabledWhenAppendingThenEnableRelaxedOrderingCorrectly, IsXeHpcCore) {
    debugManager.flags.DirectSubmissionRelaxedOrdering.set(1);
...
HWCMDTEST_F(IGFX_XE_HP_CORE, InOrderCmdListTests, givenInOrderModeWhenProgrammingWalkerThenSignalSyncAllocation) {
    ...
    *hostAddress = expectedEventValue;
    EXPECT_EQ(ZE_RESULT_SUCCESS, events[0]->hostSynchronize(1));
...
HWCMDTEST_F(IGFX_XE_HP_CORE, InOrderCmdListTests, givenInOrderModeWhenProgrammingAppendWaitOnEventsThenSignalSyncAllocation) {
    ...
    // implicit dependency: either semaphore wait on in-order counter or a stalling barrier
    auto itor = cmdList.begin();
    if (genCmdCast<typename FamilyType::StallingBarrierType *>(*itor)) {
        itor++;
    } else {
        const bool lriRequired = NEO::InOrderProgrammingHelpers::isLriFor64bDataProgrammingRequired(immCmdList->isQwordInOrderCounter(), immCmdList->getDevice()->getDeviceInfo().semaphore64bCmdSupport);
        if (lriRequired) {
            std::advance(semaphoreItor, -2);
        }
...
HWCMDTEST_F(IGFX_XE_HP_CORE, InOrderCmdListTests, givenInOrderModeWhenProgrammingCounterWithOverflowThenHandleOffsetCorrectly) {
    ...
    for (uint32_t i = 0; i < 10; i++) {
        ...
        EXPECT_EQ(expectedCounter, events[0]->getInOrderExecBaseSignalValue());
        EXPECT_EQ(expectedOffset, events[0]->getInOrderAllocationOffset());
...
HWCMDTEST_F(IGFX_XE_HP_CORE, InOrderCmdListTests, givenInOrderModeWhenProgrammingCounterWithOverflowThenHandleItCorrectly) {
    ...
    EXPECT_EQ(expectedCounter, events[0]->getInOrderExecBaseSignalValue());
    EXPECT_EQ(offset, events[0]->getInOrderAllocationOffset());
...
HWTEST_F(InOrderCmdListTests, givenCorrectInputParamsWhenCreatingCbEventThenReturnSuccess) {
    ...
    *hostAddress = counterValue;
    ...
    EXPECT_EQ(ZE_RESULT_SUCCESS, L0::zexCounterBasedEventCreate(context, device, nullptr, hostAddress, counterValue, &eventDesc, &handle));
    eventObj = Event::fromHandle(handle);
    EXPECT_FALSE(eventObj->getInOrderExecEventHelper().isDataAssigned());
    zeEventDestroy(handle);

    EXPECT_EQ(ZE_RESULT_SUCCESS, L0::zexCounterBasedEventCreate(context, device, nullptr, nullptr, counterValue, &eventDesc, &handle));
    eventObj = Event::fromHandle(handle);
    EXPECT_FALSE(eventObj->getInOrderExecEventHelper().isDataAssigned());
    zeEventDestroy(handle);
...
HWTEST_F(InOrderCmdListTests, givenExternalSyncStorageWhenCreatingCounterBasedEventAndAdditionalTimestampNodeThenSetAdditionalParamsVectorCorrectly) {
    ...
    EXPECT_EQ(2u, event->inOrderExecHelper.getAdditionalTimestampNodesCount());
...
HWTEST_F(InOrderCmdListTests, givenExternalSyncStorageWhenCallingAppendThenAggregateTimestampNodes) {
    ...
    auto node0 = static_cast<NEO::TimestampPackets<TagSizeT, 1> *>(eventObj->inOrderExecHelper.getTimestampNode(0)->getCpuBase());
    auto node1 = static_cast<NEO::TimestampPackets<TagSizeT, 1> *>(eventObj->inOrderExecHelper.getTimestampNode(1)->getCpuBase());
...
HWTEST_F(InOrderCmdListTests, givenExternalSyncStorageWhenCallingAppendThenHandleResidency) {
    ...
    auto node0 = eventObj->inOrderExecHelper.getTimestampNode(0);
    ...
    auto node1 = eventObj->inOrderExecHelper.getTimestampNode(1);
...
HWTEST_F(InOrderCmdListTests, givenExternalSyncStorageWhenCallingAppendMoreThanCounterValueThenDontResetNodes) {
    ...
    immCmdList->appendLaunchKernel(kernel->toHandle(), groupCount, handle, 0, nullptr, launchParams);
    immCmdList->appendLaunchKernel(kernel->toHandle(), groupCount, handle, 0, nullptr, launchParams);
    ASSERT_EQ(3u, eventObj->inOrderExecHelper.getTimestampNodesCount());
...
HWTEST_F(InOrderCmdListTests, givenExternalSyncStorageWhenCallingSplitAppendThenSetCorrectGpuVa) {
    ...
    ASSERT_EQ(1u, eventObj->inOrderExecHelper.getTimestampNodesCount());
    auto expectedAddress0 = eventObj->inOrderExecHelper.getTimestampNode(0)->getGpuAddress() + eventObj->getGlobalStartOffset();
    ...
    immCmdList->appendMemoryCopy(unalignedPtr, unalignedPtr, ptrBaseSize - copyOffset, handle, 0, nullptr, copyParams);
    ASSERT_EQ(2u, eventObj->inOrderExecHelper.getTimestampNodesCount());
    auto expectedAddress1 = eventObj->inOrderExecHelper.getTimestampNode(1)->getGpuAddress() + eventObj->getGlobalStartOffset();
...
HWTEST_F(InOrderCmdListTests, givenCounterBasedEventAndAdditionalTimestampNodeAndResetCalledThenVectorIsCleared) {
    ...
    EXPECT_EQ(1u, events[0]->inOrderExecHelper.getAdditionalTimestampNodesCount());
