...
class SysmanDeviceMemoryFixture : public SysmanDeviceFixture {
  protected:
    ...
    void SetUp() override {
        ...
        stepping = NEO::HwInfoConfig::get(productFamily)->getSteppingFromHwRevId(hwInfo);
        getMemoryHandles(0);
    }
...
HWTEST2_F(SysmanDeviceMemoryFixture, GivenValidMemoryHandleWhenCallingzesSysmanMemoryGetBandwidthWhenVFID0IsActiveThenSuccessIsReturnedAndBandwidthIsValid, IsPVC) {
    ...
    for (auto &handle : handles) {
        ...
        hwInfo->platform.usRevId = NEO::HwInfoConfig::get(productFamily)->getHwRevIdFromStepping(REVISION_B, *hwInfo);
