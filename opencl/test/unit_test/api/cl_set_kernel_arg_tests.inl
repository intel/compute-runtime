...
TEST_F(clSetKernelArgImmediateTests, GivenImmediateArgAndArgSizeLargerThanRequiredWhenSettingKernelArgThenInvalidArgSizeIsReturned) {
    uint64_t argValue = 0x1234567890abcdef;

    auto retVal = clSetKernelArg(pMockMultiDeviceKernel, 0, sizeof(uint64_t), &argValue);
    EXPECT_EQ(CL_INVALID_ARG_SIZE, retVal);
}
...
TEST_F(clSetKernelArgImmediateMultiElementTests, GivenMultiElementImmediateArgAndArgSizeLargerThanRequiredWhenSettingKernelArgThenInvalidArgSizeIsReturned) {
    struct {
        uint32_t a;
        uint32_t b;
        uint32_t c;
