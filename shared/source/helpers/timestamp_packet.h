...
struct TimestampPacketHelper {
    ...
    static void programSemaphore(LinearStream &cmdStream, TagNodeBase &timestampPacketNode) {
        ...
        for (uint32_t packetId = 0; packetId < timestampPacketNode.getPacketsUsed(); packetId++) {
            ...
            EncodeSemaphore<GfxFamily>::addMiSemaphoreWaitCommand(cmdStream, compareAddress + compareOffset, TimestampPacketConstants::initValue, COMPARE_OPERATION::COMPARE_OPERATION_SAD_NOT_EQUAL_SDD, false, false);
        }
    }
