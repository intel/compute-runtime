...
HWTEST_F(InOrderIpcTests, givenCounterOffsetWhenOpenIsCalledThenPassCorrectData) {
    ...
    zex_ipc_counter_based_event_handle_t zexIpcData = {};
    EXPECT_EQ(ZE_RESULT_SUCCESS, L0::zexCounterBasedEventGetIpcHandle(events[1]->toHandle(), &zexIpcData));
    ...
    EXPECT_TRUE(sharableEventdataHelper.allocationOffset == ipcData.communicationAllocOffset);
