...
TEST_F(SysmanNlApiFixture, GivenBadLibraryWhenCallingLoadEntryPointsVerifyLoadEntryPointsReturnsFailure) {
    PublicNlApi *pPublicNlApi = new PublicNlApi;
    // Open the wrong library so that non of the symbols will be found
    pPublicNlApi->genlLibraryHandle = NEO::OsLibrary::load("libc.so.6");
    EXPECT_FALSE(pPublicNlApi->loadEntryPoints());
    delete pPublicNlApi->genlLibraryHandle;
    pPublicNlApi->genlLibraryHandle = nullptr;
    delete pPublicNlApi;
}
