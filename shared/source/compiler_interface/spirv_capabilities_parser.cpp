...
std::string SpirvCapabilitiesParser::getSpirvExtensionsYAMLFromDeviceCtx(NEO::IgcOclDeviceCtxTag *deviceCtx, CIF::CIFMain *igcMain) {
    if ((deviceCtx == nullptr) || (igcMain == nullptr)) {
        return "";
    }

    // Availability is decided by the handle itself: GetIgcOptionsAndCapabilitiesHandle()
    // returns nullptr when IGC does not support the SPIR-V queries interface.
