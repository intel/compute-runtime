...
if(NOT NEO_SKIP_SHARED_UNIT_TESTS)
  if(NOT skip_shared)
    ...
    if(NOT NEO_SKIP_MT_UNIT_TESTS)
      ...
      set(NEO_SHARED_MT_TESTS_ENV ${GTEST_ENV})
      if(USE_TSAN)
        set(NEO_SHARED_MT_TESTS_ENV ${NEO_SHARED_MT_TESTS_ENV} TSAN_OPTIONS=suppressions=${NEO_SOURCE_DIR}/shared/test/unit_test/mt_tests/tsan_suppressions.txt)
      endif()
      ...
      add_custom_command(
                         ...
                         COMMAND echo Cmd line: ${NEO_SHARED_MT_TESTS_ENV} ${NEO_RUN_INTERCEPTOR_LIST} $<TARGET_FILE:neo_shared_mt_tests> --product ${product} --slices ${slices} --subslices ${subslices} --eu_per_ss ${eu_per_ss} ${GTEST_EXCEPTION_OPTIONS} --gtest_repeat=${GTEST_REPEAT} ${GTEST_SHUFFLE} ${GTEST_OUTPUT} ${NEO_TESTS_LISTENER_OPTION} ${GTEST_FILTER_OPTION} --rev_id ${revision_id} --dev_id ${device_id}
                         COMMAND ${NEO_SHARED_MT_TESTS_ENV} ${NEO_RUN_INTERCEPTOR_LIST} $<TARGET_FILE:neo_shared_mt_tests> --product ${product} --slices ${slices} --subslices ${subslices} --eu_per_ss ${eu_per_ss} ${GTEST_EXCEPTION_OPTIONS} --gtest_repeat=${GTEST_REPEAT} ${GTEST_SHUFFLE} ${GTEST_OUTPUT} ${NEO_TESTS_LISTENER_OPTION} ${GTEST_FILTER_OPTION} --rev_id ${revision_id} --dev_id ${device_id}
      )
...
if(NOT NEO_SKIP_OCL_UNIT_TESTS)
  if(NOT skip_ocl)
    ...
    set(NEO_OCL_TESTS_CMD_PREFIX)
    if(USE_TSAN)
      set(NEO_OCL_TESTS_CMD_PREFIX ${CMAKE_COMMAND} -E env "TSAN_OPTIONS=suppressions=${NEO_SOURCE_DIR}/opencl/test/unit_test/mt_tests/tsan_suppressions.txt")
    endif()
    ...
    add_custom_command(
                       ...
                       COMMAND echo Cmd line: ${NEO_OCL_TESTS_CMD_PREFIX} ${GTEST_ENV} ${NEO_RUN_INTERCEPTOR_LIST} $<TARGET_FILE:igdrcl_tests> --product ${product} --slices ${slices} --subslices ${subslices} --eu_per_ss ${eu_per_ss} ${GTEST_EXCEPTION_OPTIONS} --gtest_repeat=${GTEST_REPEAT} ${GTEST_SHUFFLE} ${GTEST_OUTPUT} ${NEO_TESTS_LISTENER_OPTION} ${GTEST_FILTER_OPTION} --rev_id ${revision_id} --dev_id ${device_id}
                       COMMAND ${NEO_OCL_TESTS_CMD_PREFIX} ${GTEST_ENV} ${NEO_RUN_INTERCEPTOR_LIST} $<TARGET_FILE:igdrcl_tests> --product ${product} --slices ${slices} --subslices ${subslices} --eu_per_ss ${eu_per_ss} ${GTEST_EXCEPTION_OPTIONS} --gtest_repeat=${GTEST_REPEAT} ${GTEST_SHUFFLE} ${GTEST_OUTPUT} ${NEO_TESTS_LISTENER_OPTION} ${GTEST_FILTER_OPTION} --rev_id ${revision_id} --dev_id ${device_id}
    )
    ...
    if(NOT NEO_SKIP_MT_UNIT_TESTS)
      unset(GTEST_OUTPUT)
      if(DEFINED GTEST_OUTPUT_DIR)
        set(GTEST_OUTPUT "--gtest_output=json:${GTEST_OUTPUT_DIR}/ocl-${product}-${revision_id}-mt_tests-results.json")
      endif()

      if(NOT TARGET run_ocl_mt_tests)
        add_custom_target(run_ocl_mt_tests)
      endif()
      set_target_properties(run_ocl_mt_tests PROPERTIES FOLDER ${PLATFORM_SPECIFIC_TEST_TARGETS_FOLDER})
      add_dependencies(run_mt_unit_tests run_ocl_mt_tests)

      add_custom_target(run_${product}_${revision_id}_ocl_mt_tests DEPENDS igdrcl_mt_tests)
      set_target_properties(run_${product}_${revision_id}_ocl_mt_tests PROPERTIES FOLDER "${PLATFORM_SPECIFIC_TEST_TARGETS_FOLDER}/${product}/${revision_id}")

      set(NEO_OCL_MT_TESTS_ENV ${GTEST_ENV})
      if(USE_TSAN)
        set(NEO_OCL_MT_TESTS_ENV ${NEO_OCL_MT_TESTS_ENV} TSAN_OPTIONS=suppressions=${NEO_SOURCE_DIR}/opencl/test/unit_test/mt_tests/tsan_suppressions.txt)
      endif()

      add_custom_command(
                         TARGET run_${product}_${revision_id}_ocl_mt_tests
                         POST_BUILD
                         COMMAND WORKING_DIRECTORY ${TargetDir}
                         COMMAND ${CMAKE_COMMAND} -E make_directory ${TargetDir}/opencl/${product}/${revision_id}
                         COMMAND echo Running igdrcl_mt_tests ${product} ${slices}x${subslices}x${eu_per_ss} in ${TargetDir}
                         COMMAND echo Cmd line: ${NEO_OCL_MT_TESTS_ENV} ${NEO_RUN_INTERCEPTOR_LIST} $<TARGET_FILE:igdrcl_mt_tests> --product ${product} --slices ${slices} --subslices ${subslices} --eu_per_ss ${eu_per_ss} ${GTEST_EXCEPTION_OPTIONS} --gtest_repeat=${GTEST_REPEAT} ${GTEST_SHUFFLE} ${GTEST_OUTPUT} ${NEO_TESTS_LISTENER_OPTION} ${GTEST_FILTER_OPTION} --rev_id ${revision_id} --dev_id ${device_id}
                         COMMAND ${NEO_OCL_MT_TESTS_ENV} ${NEO_RUN_INTERCEPTOR_LIST} $<TARGET_FILE:igdrcl_mt_tests> --product ${product} --slices ${slices} --subslices ${subslices} --eu_per_ss ${eu_per_ss} ${GTEST_EXCEPTION_OPTIONS} --gtest_repeat=${GTEST_REPEAT} ${GTEST_SHUFFLE} ${GTEST_OUTPUT} ${NEO_TESTS_LISTENER_OPTION} ${GTEST_FILTER_OPTION} --rev_id ${revision_id} --dev_id ${device_id}
      )

      add_dependencies(run_ocl_mt_tests run_${product}_${revision_id}_ocl_mt_tests)
    endif()
  endif()
endif()
...
if(NOT NEO_SKIP_L0_UNIT_TESTS AND BUILD_WITH_L0)
  if(NOT skip_l0)
    ...
    if(NOT NEO_SKIP_MT_UNIT_TESTS)
      ...
      set(NEO_L0_MT_TESTS_CMD_PREFIX)
      if(USE_TSAN)
        set(NEO_L0_MT_TESTS_CMD_PREFIX ${CMAKE_COMMAND} -E env "TSAN_OPTIONS=suppressions=${NEO_SOURCE_DIR}/level_zero/core/test/unit_tests/mt_tests/tsan_suppressions.txt")
      endif()
      ...
      add_custom_command(
                         ...
                         COMMAND echo Cmd line: ${NEO_L0_MT_TESTS_CMD_PREFIX} ${NEO_RUN_INTERCEPTOR_LIST} $<TARGET_FILE:ze_intel_gpu_core_mt_tests> --product ${product} --slices ${slices} --subslices ${subslices} --eu_per_ss ${eu_per_ss} ${GTEST_EXCEPTION_OPTIONS} --gtest_repeat=${GTEST_REPEAT} ${GTEST_SHUFFLE} ${GTEST_OUTPUT_L0MT} ${NEO_TESTS_LISTENER_OPTION} ${GTEST_FILTER_OPTION} --rev_id ${revision_id} --dev_id ${device_id}
                         COMMAND ${NEO_L0_MT_TESTS_CMD_PREFIX} ${NEO_RUN_INTERCEPTOR_LIST} $<TARGET_FILE:ze_intel_gpu_core_mt_tests> --product ${product} --slices ${slices} --subslices ${subslices} --eu_per_ss ${eu_per_ss} ${GTEST_EXCEPTION_OPTIONS} --gtest_repeat=${GTEST_REPEAT} ${GTEST_SHUFFLE} ${GTEST_OUTPUT_L0MT} ${NEO_TESTS_LISTENER_OPTION} ${GTEST_FILTER_OPTION} --rev_id ${revision_id} --dev_id ${device_id}
                         COMMAND echo Running ze_intel_gpu_tools_mt_tests ${target} ${slices}x${subslices}x${eu_per_ss} in ${TargetDir}
                         COMMAND echo Cmd line: ${NEO_L0_MT_TESTS_CMD_PREFIX} ${NEO_RUN_INTERCEPTOR_LIST} $<TARGET_FILE:ze_intel_gpu_tools_mt_tests> --product ${product} --slices ${slices} --subslices ${subslices} --eu_per_ss ${eu_per_ss} ${GTEST_EXCEPTION_OPTIONS} --gtest_repeat=${GTEST_REPEAT} ${GTEST_SHUFFLE} ${GTEST_OUTPUT_TOOLS_MT} ${NEO_TESTS_LISTENER_OPTION} ${GTEST_FILTER_OPTION} --rev_id ${revision_id} --dev_id ${device_id}
                         COMMAND ${NEO_L0_MT_TESTS_CMD_PREFIX} ${NEO_RUN_INTERCEPTOR_LIST} $<TARGET_FILE:ze_intel_gpu_tools_mt_tests> --product ${product} --slices ${slices} --subslices ${subslices} --eu_per_ss ${eu_per_ss} ${GTEST_EXCEPTION_OPTIONS} --gtest_repeat=${GTEST_REPEAT} ${GTEST_SHUFFLE} ${GTEST_OUTPUT_TOOLS_MT} ${NEO_TESTS_LISTENER_OPTION} ${GTEST_FILTER_OPTION} --rev_id ${revision_id} --dev_id ${device_id}
      )
