...
ze_result_t DriverHandleImp::formatRTASCompatibilityCheck(ze_rtas_format_exp_t rtasFormatA,
    ...
    if (this->rtasLibraryHandle == nullptr) {
        this->rtasLibraryHandle = RTASBuilder::osLibraryLoadFunction(RTASBuilder::rtasLibraryName);
        if (this->rtasLibraryHandle == nullptr || RTASBuilder::loadEntryPoints(this->rtasLibraryHandle) == false) {
...
ze_result_t DriverHandleImp::createRTASBuilder(const ze_rtas_builder_exp_desc_t *desc,
    ...
    ze_result_t result = builderCreateExpImpl(this->toHandle(), desc, &pRTASBuilder->handleImpl);
    if (result != ZE_RESULT_SUCCESS) {
        delete pRTASBuilder;
        return result;
    }

    *phBuilder = pRTASBuilder->toHandle();
...
ze_result_t DriverHandleImp::createRTASParallelOperation(ze_rtas_parallel_operation_exp_handle_t *phParallelOperation) {
    RTASParallelOperation *pRTASParallelOperation = new RTASParallelOperation();
    ...
    if (this->rtasLibraryHandle == nullptr) {
        ...
        if (this->rtasLibraryHandle == nullptr || RTASBuilder::loadEntryPoints(this->rtasLibraryHandle) == false) {
            ...
            delete pRTASParallelOperation;
            PRINT_DEBUG_STRING(NEO::DebugManager.flags.PrintDebugMessages.get(), stderr, "Failed to load Ray Tracing Support Library %s\n", RTASBuilder::rtasLibraryName.c_str());
            return ZE_RESULT_ERROR_DEPENDENCY_UNAVAILABLE;
        }
    }

    ze_result_t result = parallelOperationCreateExpImpl(this->toHandle(), &pRTASParallelOperation->handleImpl);
    if (result != ZE_RESULT_SUCCESS) {
        delete pRTASParallelOperation;
        return result;
    }

    *phParallelOperation = pRTASParallelOperation->toHandle();
    return ZE_RESULT_SUCCESS;
}
