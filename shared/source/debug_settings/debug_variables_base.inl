...
DECLARE_DEBUG_VARIABLE(bool, EnableipcSupportedAllocationByDefault, false, "Enable creating shareable allocations without NT handle on Windows for IPC Support By Default")
...
DECLARE_DEBUG_VARIABLE(int32_t, ForceCsrLockInBcsEnqueueOnlyForGpgpuSubmission, -1, "-1: Default (always acquire gpgpu CSR lock in bcs enqueue), 0: Do not acquire gpgpu CSR lock in bcs enqueue, 1: Same as default")
...
DECLARE_DEBUG_VARIABLE(int32_t, EventHostSynchronizeWaitStrategy, 3, "Power/CPU efficiency mode for zeEventHostSynchronize on WDDM. 0: disabled, 1: sleep strategy enabled, 2: sleep strategy enabled only when AC line is disconnected, 3: KMD wait after initial polling")
...
DECLARE_DEBUG_VARIABLE(std::string, OverrideIgdrclLogFileName, std::string("unk"), "Overrides igdrcl logger output file name; ignored when unk")
...
DECLARE_DEBUG_VARIABLE(int32_t, SplitBcsForRegularCmdList, -1, "-1: default, 0: disabled, 1: enable BCS split for regular cmd lists")
...
DECLARE_DEBUG_VARIABLE(int32_t, OverrideFastModePoll, -1, "Override MI_SEMAPHORE_WAIT_64 fast mode poll bit. -1: default (disable), 0: disable, 1: enable")
...
DECLARE_DEBUG_VARIABLE(int64_t, DirectSubmissionInitialSemaphoreValue, -1, "-1: default, [1 ...  (uint32_t::max - 1)]: initial semaphore counter value.")
/*FEATURE FLAGS*/
...
DECLARE_DEBUG_VARIABLE(bool, EnableDivergentBarrierHandling, false, "Appends internal build option -cl-intel-enable-divergent-barrier-handling to program/module compilation. 0: disabled, 1: enabled. RebuildPrecompiledKernels debug flag might also be needed for applications shipping precompiled kernels")
