...
inline std::pair<uint32_t, uint32_t> getActiveIndices(std::array<uint8_t, 3> walkOrder, uint8_t firstDim, uint8_t secondDim) {
    auto firstIndex = UINT32_MAX;
    auto secondIndex = UINT32_MAX;
    for (auto currentDimension = 0u; currentDimension < 3u; currentDimension++) {
        if (walkOrder[currentDimension] == firstDim) {
            firstIndex = currentDimension;
        }
        if (walkOrder[currentDimension] == secondDim) {
            secondIndex = currentDimension;
        }
    }
    UNRECOVERABLE_IF(firstIndex == UINT32_MAX || secondIndex == UINT32_MAX);
