...
#include "level_zero/api/opencl/source/sharings/gl/leo_gl_buffer.h"
...
Buffer *GlBuffer::createSharedGlBuffer(Context *context, cl_mem_flags flags, unsigned int bufferId, cl_int *errcodeRet) {
    ...
    bool success = sharingFunctions->flushObjectsAndWait(1, &objIn, &flushOut, &retValue);
    if (success) {
...
GraphicsAllocation *GlBuffer::createGraphicsAllocation(Context *context, unsigned int bufferId, _tagCLGLBufferInfo &bufferInfo) {
    ...
    auto &device = context->getClDevice()->getDevice();
    if (!graphicsAllocation) {
        AllocationProperties properties = {context->getClDevice()->getRootDeviceIndex(),
                                           false,
                                           0u,
        ...
        graphicsAllocation =
            device.getMemoryManager()->createGraphicsAllocationFromSharedHandle(osHandleData, properties, false, false, false, nullptr);
    }

    if (!graphicsAllocation) {
        return nullptr;
    }
    graphicsAllocation->incReuseCount();

    if (!reusedAllocation) {
        sharingFunctions->graphicsAllocationsForGlBufferReuse.push_back(std::make_pair(bufferId, graphicsAllocation));
        auto helper = device.getRootDeviceEnvironment().getGmmHelper();
        if (bufferInfo.pGmmResInfo) {
            DEBUG_BREAK_IF(graphicsAllocation->getDefaultGmm() != nullptr);
            auto gmmResInfo = helper->getClientContext()->getGmmResInfoFromExternalResourceHandle(bufferInfo.pGmmResInfo);
            auto gmmResourceInfo = std::unique_ptr<GmmResourceInfo>(GmmResourceInfo::create(helper->getClientContext(), gmmResInfo));
...
void GlBuffer::releaseResource(MemObj *memObject, uint32_t rootDeviceIndex) {
    auto memoryManager = memObject->getContext()->getClDevice()->getDevice().getMemoryManager();
