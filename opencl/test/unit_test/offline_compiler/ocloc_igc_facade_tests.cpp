...
class TestOclocIgcAsFcl : public NEO::OclocIgcAsFcl {
  public:
    using NEO::OclocIgcAsFcl::igc;
    TestOclocIgcAsFcl(OclocArgHelper *argHelper) : NEO::OclocIgcAsFcl(argHelper) {
        auto mockIgc = std::make_unique<NEO::MockOclocIgcFacade>(argHelper);
        mockIgc->igcDeviceCtx.reset(new NEO::MockIgcOclDeviceCtx());
        igc = std::move(mockIgc);
    }
};

TEST_F(OclocIgcFacadeTest, GivenTranslateWhenErrorExistsOrTranslationContextFailsThenNullptrIsReturned) {
    auto igcDebugVars = NEO::getIgcDebugVars();
    igcDebugVars.shouldFailCreationOfTranslationContext = true;
    NEO::setIgcDebugVars(igcDebugVars);
    TestOclocIgcAsFcl fclFacade(&mockArgHelper);

    auto mockError = std::make_unique<MockCIFBuffer>();
    auto mockSrc = std::make_unique<MockCIFBuffer>();
    auto mockOptions = std::make_unique<MockCIFBuffer>();
    auto mockInternalOptions = std::make_unique<MockCIFBuffer>();
    auto mockTracingOptions = std::make_unique<MockCIFBuffer>();

    mockError->Resize(1);
    auto ret1 = fclFacade.translate(IGC::CodeType::oclC, IGC::CodeType::spirV, mockError.get(), mockSrc.get(), mockOptions.get(), mockInternalOptions.get(), mockTracingOptions.get(), 0, 0u);
    EXPECT_EQ(nullptr, ret1.get());

    mockError->Resize(0);
    auto ret2 = fclFacade.translate(IGC::CodeType::oclC, IGC::CodeType::spirV, mockError.get(), mockSrc.get(), mockOptions.get(), mockInternalOptions.get(), mockTracingOptions.get(), 0, 0u);
    EXPECT_EQ(nullptr, ret2.get());

    igcDebugVars.shouldFailCreationOfTranslationContext = false;
    NEO::setIgcDebugVars(igcDebugVars);
}
TEST_F(OclocIgcFacadeTest, GivenTranslateWhenAllIsCorrectThenOutputAndSrcHashArePassed) {
    TestOclocIgcAsFcl fclFacade(&mockArgHelper);
    ...
    auto mockInternalOptions = std::make_unique<MockCIFBuffer>();
    auto mockTracingOptions = std::make_unique<MockCIFBuffer>();
    uint64_t expectedSrcHash = 0xcafebabedeadbeef;

    mockError->Resize(0);
    auto ret = fclFacade.translate(IGC::CodeType::oclC, IGC::CodeType::spirV, mockError.get(), mockSrc.get(), mockOptions.get(), mockInternalOptions.get(), mockTracingOptions.get(), 0, expectedSrcHash);
