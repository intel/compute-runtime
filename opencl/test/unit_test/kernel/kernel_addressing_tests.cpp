...
struct KernelAddressingTest : public ClDeviceFixture, public ::testing::Test {
    ...
    void applyWideness(bool widenessEnabled) {
        isWideness = widenessEnabled;
        prog.reset();
...
TEST_F(KernelAddressingTest, givenBuiltinCopyBufferToBufferKernelsWhenWidenessIsEnabledThenCorrectArgumentSizesAreUsed) {
    for (bool wideness : {false, true}) {
        applyWideness(wideness);
