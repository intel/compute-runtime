...
void ClDevice::initializeCaps() {
    ...
    if (debugManager.flags.OverrideDeviceName.get() != "unk") {
        ...
        if (getDriverInfo()) {
            name.assign(getDriverInfo()->getDeviceName(name).c_str());
        }
    }

    if (getDriverInfo()) {
        driverVersion.assign(getDriverInfo()->getVersion(driverVersion).c_str());
        sharingFactory.verifyExtensionSupport(getDriverInfo());
    }
    ...
    if (sharingAllowed) {
        deviceExtensions += sharingFactory.getExtensions(getDriverInfo());
    }

    PhysicalDevicePciBusInfo pciBusInfo(PhysicalDevicePciBusInfo::invalidValue, PhysicalDevicePciBusInfo::invalidValue, PhysicalDevicePciBusInfo::invalidValue, PhysicalDevicePciBusInfo::invalidValue);

    if (getDriverInfo()) {
        pciBusInfo = getDriverInfo()->getPciBusInfo();
    }
    ...
    if (!rootDeviceEnvironment.isExposeSingleDeviceMode()) {
        deviceInfo.partitionMaxSubDevices = getDevice().getNumSubDevices();
    }
    ...
    deviceInfo.nonUniformWorkGroupSupport = false;
    ...
    constexpr std::array<uint32_t, 4> supportedThreadArbitrationPolicies{CL_KERNEL_EXEC_INFO_THREAD_ARBITRATION_POLICY_OLDEST_FIRST_INTEL, CL_KERNEL_EXEC_INFO_THREAD_ARBITRATION_POLICY_ROUND_ROBIN_INTEL, CL_KERNEL_EXEC_INFO_THREAD_ARBITRATION_POLICY_AFTER_DEPENDENCY_ROUND_ROBIN_INTEL, CL_KERNEL_EXEC_INFO_THREAD_ARBITRATION_POLICY_STALL_BASED_ROUND_ROBIN_INTEL};
