...
struct CommandListCoreFamilyImmediate : public CommandListCoreFamily<gfxCoreFamily> {
    ...
    NEO::CompletionStamp flushImmediateRegularTaskHeapless(NEO::LinearStream &cmdStreamTask, size_t taskStartOffset, bool hasStallingCmds, bool hasRelaxedOrderingDependencies, NEO::AppendOperations appendOperation, bool requireTaskCountUpdate);
    ...
    MOCKABLE_VIRTUAL ze_result_t checkAvailableSpace(uint32_t numEvents, bool hasRelaxedOrderingDependencies, size_t commandSize, bool requestCommandBufferInLocalMem);
