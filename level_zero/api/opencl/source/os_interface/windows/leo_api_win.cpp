...
#include "level_zero/api/opencl/source/sharings/d3d/leo_d3d_buffer.h"
...
NEO::LEO::ClDevice *pickDeviceWithAdapterLuid(NEO::LEO::Platform *platform, LUID adapterLuid) {
    NEO::LEO::ClDevice *deviceToReturn = nullptr;
    for (int i = 0; i < std::ssize(platform->getDevices()); ++i) {
        auto device = platform->getDevices()[i].get();
        if (device->getDevice().getRootDeviceEnvironment().osInterface->getDriverModel()->as<NEO::Wddm>()->verifyAdapterLuid(adapterLuid)) {
...
void NEO::LEO::Image::getOsSpecificImageInfo(const cl_mem_info &paramName, size_t *srcParamSize, void **srcParam) {
    switch (paramName) {
...
cl_int CL_API_CALL clGetDeviceIDsFromDX9INTEL(cl_platform_id platform, cl_dx9_device_source_intel dx9DeviceSource, void *dx9Object,
    ...
    auto [retVal, platformInternal] = NEO::LEO::validateAndCast(std::make_tuple(platform));
    if (retVal != CL_SUCCESS) {
        return retVal;
    }

    cl_device_id device = platformInternal->getDevices()[0].get();
...
cl_mem CL_API_CALL clCreateFromDX9MediaSurfaceINTEL(cl_context context, cl_mem_flags flags, IDirect3DSurface9 *resource,
    ...
    auto [retVal, ctx] = NEO::LEO::validateAndCast(std::make_tuple(context));
    if (retVal != CL_SUCCESS) {
        err.set(retVal);
        return nullptr;
    }
...
cl_int CL_API_CALL clEnqueueAcquireDX9ObjectsINTEL(cl_command_queue commandQueue, cl_uint numObjects, const cl_mem *memObjects,
    ...
    auto [retVal, cmdQ] = NEO::LEO::validateAndCast(std::make_tuple(commandQueue),
                                                    std::make_tuple(MemObjList{memObjects, numObjects}, EventWaitList{eventWaitList, numEventsInWaitList}));
    if (retVal != CL_SUCCESS) {
        return retVal;
    }
...
cl_int CL_API_CALL clEnqueueReleaseDX9ObjectsINTEL(cl_command_queue commandQueue, cl_uint numObjects, cl_mem *memObjects,
                                                   cl_uint numEventsInWaitList, const cl_event *eventWaitList, cl_event *event) {
    auto [retVal, cmdQ] = NEO::LEO::validateAndCast(std::make_tuple(commandQueue),
                                                    std::make_tuple(MemObjList{const_cast<const cl_mem *>(memObjects), numObjects}, EventWaitList{eventWaitList, numEventsInWaitList}));
    if (retVal != CL_SUCCESS) {
        return retVal;
    }
    ...
    auto ret = cmdQ->enqueueReleaseSharedObjects(numObjects, memObjects, numEventsInWaitList,
...
cl_int CL_API_CALL clGetDeviceIDsFromDX9MediaAdapterKHR(cl_platform_id platform, cl_uint numMediaAdapters,
    ...
    return clGetDeviceIDsFromDX9INTEL(platform, 0, nullptr, 0, numEntries, devices, numDevices);
}
...
cl_int CL_API_CALL clGetDeviceIDsFromD3D10KHR(cl_platform_id platform, cl_d3d10_device_source_khr d3dDeviceSource,
    ...
    auto [retVal, platformInternal] = NEO::LEO::validateAndCast(std::make_tuple(platform));
    NEO::LEO::ClDevice *device = nullptr;
    if (retVal != CL_SUCCESS) {
        return retVal;
    }

    if (NEO::debugManager.injectFcn) {
        sharingFcns.getDxgiDescFcn = (D3DSharingFunctions<D3DTypesHelper::D3D10>::GetDxgiDescFcn)NEO::debugManager.injectFcn;
    }
...
cl_int CL_API_CALL clEnqueueAcquireD3D10ObjectsKHR(cl_command_queue commandQueue, cl_uint numObjects, const cl_mem *memObjects,
    ...
    for (unsigned int object = 0; object < numObjects; object++) {
        auto memObj = NEO::LEO::castToObject<NEO::LEO::MemObj>(memObjects[object]);
...
cl_int CL_API_CALL clGetDeviceIDsFromD3D11KHR(cl_platform_id platform, cl_d3d11_device_source_khr d3dDeviceSource,
    ...
    if (NEO::debugManager.injectFcn) {
        sharingFcns.getDxgiDescFcn = (D3DSharingFunctions<D3DTypesHelper::D3D11>::GetDxgiDescFcn)NEO::debugManager.injectFcn;
    }
