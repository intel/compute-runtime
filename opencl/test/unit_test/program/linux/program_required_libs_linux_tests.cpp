...
struct ProgramRequiredLibsLinuxTest : public Test<ClDeviceFixture> {
    void SetUp() override {
...
TEST_F(ProgramRequiredLibsLinuxTest, givenInvalidLibBinaryWhenGetRequiredLibProgramCalledThenReturnsNullAndDoesNotCache) {
    DebugManagerStateRestore restorer;
    debugManager.flags.RequiredLibsBinarySearchPath.set(mockedSearchPath);

    char invalidBinary[] = {0, 0, 0, 0};
    IoFunctions::mockFreadBuffer = invalidBinary;
    IoFunctions::mockFreadReturn = sizeof(invalidBinary);
    IoFunctions::mockFtellReturn = sizeof(invalidBinary);

    constexpr auto libName = "libInvalid.zebin";
    EXPECT_EQ(nullptr, pClDevice->getRequiredLibProgram(libName));
