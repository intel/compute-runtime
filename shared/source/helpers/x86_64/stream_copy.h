...
inline void streamCopy(void *dst, const void *src, size_t bytes, bool srcHeadBlockReadable) noexcept {
    ...
    if (bytes >= streamCopyAvx512Width && cpuInfo.isFeatureSupported(CpuInfo::featureAvX512)) {
        streamCopyFromWriteCombinedAvx512(dst, src, bytes, srcHeadBlockReadable);
    } else if (bytes >= streamCopyAvx2Width && cpuInfo.isFeatureSupported(CpuInfo::featureAvX2)) {
        streamCopyFromWriteCombinedAvx2(dst, src, bytes, srcHeadBlockReadable);
    } else if (bytes >= streamCopySseWidth && cpuInfo.isFeatureSupported(CpuInfo::featureSse41)) {
        streamCopyFromWriteCombinedSse(dst, src, bytes, srcHeadBlockReadable);
    } else {
