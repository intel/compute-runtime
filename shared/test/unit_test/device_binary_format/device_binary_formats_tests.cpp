...
TEST(UnpackSingleDeviceBinary, GivenArBinaryThenDelegatesToArchiveUnpacker) {
    ZebinTestData::ValidEmptyProgram zebin;
    NEO::Ar::ArEncoder encoder{true};
    const std::string requiredProduct = "unk";
    ASSERT_TRUE(encoder.appendFileEntry("64." + requiredProduct, zebin.storage));
    ...
    auto unpacked = NEO::unpackSingleDeviceBinary(arData, requiredProduct, target, outErrors, outWarnings);
    EXPECT_TRUE(outErrors.empty());
    EXPECT_FALSE(unpacked.deviceBinary.empty());
}
