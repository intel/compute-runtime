...
struct ReleaseHelperDg2G12Tests : public ReleaseHelperTests<12, 57> {
...
TEST_F(ReleaseHelperDg2G12Tests, whenGettingCapabilitiesThenCorrectPropertiesAreReturned) {
...
TEST_F(ReleaseHelperDg2G12Tests, whenGettingSupportedNumGrfsThenCorrectValuesAreReturned) {
    whenGettingSupportedNumGrfsThenValues128And256Returned();
}

TEST_F(ReleaseHelperDg2G12Tests, whenGettingThreadsPerEuConfigsThen4And8AreReturned) {
...
TEST_F(ReleaseHelperDg2G12Tests, whenGettingAdditionalFp16AtomicCapabilitiesThenReturnNoCapabilities) {
    whenGettingAdditionalFp16AtomicCapabilitiesThenReturnNoCapabilities();
}

TEST_F(ReleaseHelperDg2G12Tests, whenGettingAdditionalExtraKernelCapabilitiesThenReturnNoCapabilities) {
...
TEST_F(ReleaseHelperDg2G12Tests, whenProgrammAdditionalStallPriorToBarrierWithTimestampCalledThenFalseReturned) {
    whenProgrammAdditionalStallPriorToBarrierWithTimestampCalledThenFalseReturned();
}

TEST_F(ReleaseHelperDg2G12Tests, whenIsPostImageWriteFlushRequiredCalledThenTrueReturned) {
    for (auto &revision : getRevisions()) {
        ipVersion.revision = revision;
        releaseHelper = ReleaseHelper::create(ipVersion);
        ASSERT_NE(nullptr, releaseHelper);
...
TEST_F(ReleaseHelperDg2G12Tests, whenIsPreImageReadFlushRequiredCalledThenFalseReturned) {
    whenIsPreImageReadFlushRequiredCalledThenFalseReturned();
}

TEST_F(ReleaseHelperDg2G12Tests, whenGettingPreferredSlmSizeThenAllEntriesHaveCorrectValues) {
    for (auto &revision : getRevisions()) {
        ipVersion.revision = revision;
        releaseHelper = ReleaseHelper::create(ipVersion);
        ASSERT_NE(nullptr, releaseHelper);
...
TEST_F(ReleaseHelperDg2G12Tests, whenCallingAdjustMaxThreadsPerEuCountThenCorrectValueIsReturned) {
    whenCallingAdjustMaxThreadsPerEuCountThenCorrectValueIsReturned();
}

TEST_F(ReleaseHelperDg2G12Tests, whenShouldQueryPeerAccessCalledThenFalseReturned) {
    whenShouldQueryPeerAccessCalledThenFalseReturned();
}

TEST_F(ReleaseHelperDg2G12Tests, whenIsSingleDispatchRequiredForMultiCCSCalledThenFalseReturned) {
    whenIsSingleDispatchRequiredForMultiCCSCalledThenFalseReturned();
}

TEST_F(ReleaseHelperDg2G12Tests, whenIsStateCacheInvalidationWaRequiredCalledThenFalseReturned) {
    whenIsStateCacheInvalidationWaRequiredCalledThenFalseReturned();
}

TEST_F(ReleaseHelperDg2G12Tests, whenNumberOfCCSEnabledLargerThanOneAndIsRcsFalseThenPipeControlPriorToNonPipelinedStateCommandsExtendedWARequiredCalledThenTrueReturned) {
...
TEST_F(ReleaseHelperDg2G12Tests, whenNumberOfCCSEnabledLargerThanOneAndIsRcsTrueThenPipeControlPriorToNonPipelinedStateCommandsExtendedWARequiredCalledThenFalseReturned) {
    HardwareInfo hwInfo = *defaultHwInfo;
    hwInfo.gtSystemInfo.CCSInfo.NumberOfCCSEnabled = 2;
    releaseHelper = ReleaseHelper::create(ipVersion);
...
TEST_F(ReleaseHelperDg2G12Tests, whenNumberOfCCSEnabledEqualToOneAndIsRcsFalseThenPipeControlPriorToNonPipelinedStateCommandsExtendedWARequiredCalledThenFalseReturned) {
    HardwareInfo hwInfo = *defaultHwInfo;
...
TEST_F(ReleaseHelperDg2G12Tests, whenNumberOfCCSEnabledEqualToOneAndIsRcsTrueThenPipeControlPriorToNonPipelinedStateCommandsExtendedWARequiredCalledThenFalseReturned) {
    HardwareInfo hwInfo = *defaultHwInfo;
    hwInfo.gtSystemInfo.CCSInfo.NumberOfCCSEnabled = 1;
    releaseHelper = ReleaseHelper::create(ipVersion);
    EXPECT_FALSE(releaseHelper->isPipeControlPriorToNonPipelinedStateCommandsExtendedWARequired(hwInfo, true));
