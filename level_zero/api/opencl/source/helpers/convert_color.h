...
inline cl_image_format redescribeFillColor(const ImageInfo &imageInfo) {
    auto numChannels = imageInfo.surfaceFormat->numChannels;
    auto perChannelSize = imageInfo.surfaceFormat->perChannelSizeInBytes;

    cl_channel_order redescribedOrder = (numChannels <= 1) ? CL_R : (numChannels <= 2) ? CL_RG
                                                                                       : CL_RGBA;
    cl_channel_type redescribedType = (perChannelSize <= 1) ? CL_UNSIGNED_INT8 : (perChannelSize <= 2) ? CL_UNSIGNED_INT16
                                                                                                       : CL_UNSIGNED_INT32;
    return cl_image_format{redescribedOrder, redescribedType};
}
} // namespace NEO
