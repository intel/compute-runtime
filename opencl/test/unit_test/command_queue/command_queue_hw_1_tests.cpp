...
HWTEST_F(CommandQueueHwTest, GivenEventWithRootDeviceSyncNodesWhenProcessDispatchForMarkerCalledThenEndNodeWillBeSignaledByPipeControl) {

    pDevice->getUltCommandStreamReceiver<FamilyType>().timestampPacketWriteEnabled = false;
    MockCommandStreamReceiverWithFailingFlushBatchedSubmission csr(*pDevice->getExecutionEnvironment(), 0, pDevice->getDeviceBitfield());
    auto mockTagAllocator = std::make_unique<MockTagAllocator<>>(pCmdQ->getContextPtr()->getRootDeviceIndices(), pDevice->getExecutionEnvironment()->memoryManager.get(), 10u);
    std::unique_ptr<TagAllocatorBase> uniquePtr(mockTagAllocator.release());
    pCmdQ->getContextPtr()->setMultiRootDeviceTimestampPacketAllocator(uniquePtr);
    auto myCmdQ = std::make_unique<MockCommandQueueHwWithOverwrittenCsr<FamilyType>>(pCmdQ->getContextPtr(), pClDevice, nullptr, false);
    myCmdQ->csr = &csr;
    csr.osContext = &pCmdQ->getGpgpuCommandStreamReceiver().getOsContext();
    std::unique_ptr<Event> event(new Event(myCmdQ.get(), CL_COMMAND_COPY_BUFFER, 0, 0));
    ...
    event->getMultiRootTimestampSyncNode();
    auto lastNode = event->getMultiRootTimestampSyncNode();
    ASSERT_NE(nullptr, event);

    cl_event clEvent = event.get();
    EventsRequest eventsRequest(0, nullptr, &clEvent);
    uint32_t streamBuffer[100] = {};
    NEO::LinearStream linearStream(streamBuffer, sizeof(streamBuffer));
    CsrDependencies deps = {};
    myCmdQ->processDispatchForMarker(*myCmdQ.get(), &linearStream, eventsRequest, deps);
    ...
    ASSERT_TRUE(FamilyType::PARSE::parseCommandBuffer(
        cmdList, ptrOffset(linearStream.getCpuBase(), 0), linearStream.getUsed()));
    ...
    while (itor != cmdList.end()) {
        ...
        if (address == lastNode->getGpuAddress() + lastNode->getContextEndOffset()) {
            expectedAddressInPipeControl = true;
            break;
        }
        itor = find<typename FamilyType::PIPE_CONTROL *>(++itor, cmdList.end());
    }
    EXPECT_TRUE(expectedAddressInPipeControl);
}

HWTEST_F(CommandQueueHwTest, GivenEventWithEmptyRootDeviceSyncNodesContainerWhenProcessDispatchForMarkerCalledThenSyncPipeControlIsNotProgrammed) {
    ...
    myCmdQ->csr = &csr;
    csr.osContext = &pCmdQ->getGpgpuCommandStreamReceiver().getOsContext();
    std::unique_ptr<MockEvent<Event>> event(new MockEvent<Event>(myCmdQ.get(), CL_COMMAND_COPY_BUFFER, 0, 0));
    auto node1 = event->getMultiRootTimestampSyncNode();
    auto node2 = event->getMultiRootTimestampSyncNode();
    auto node3 = event->getMultiRootTimestampSyncNode();
    event->multiRootDeviceTimestampPacketContainer.reset(new TimestampPacketContainer());
    ASSERT_NE(nullptr, event);
    ...
    ASSERT_TRUE(FamilyType::PARSE::parseCommandBuffer(
        cmdList, ptrOffset(linearStream.getCpuBase(), 0), linearStream.getUsed()));
    auto itor = find<typename FamilyType::PIPE_CONTROL *>(cmdList.begin(), cmdList.end());
    bool notExpectedAddressInPipeControl = true;
    while (itor != cmdList.end()) {
        auto pipeControlCmd = reinterpret_cast<typename FamilyType::PIPE_CONTROL *>(*itor);
        uint64_t addressHigh = pipeControlCmd->getAddressHigh();
        uint64_t addressLow = pipeControlCmd->getAddress();
        addressHigh = addressHigh << 32;
        uint64_t address = addressHigh | addressLow;
        if (address == node1->getGpuAddress() + node1->getContextEndOffset() ||
            address == node2->getGpuAddress() + node2->getContextEndOffset() ||
            address == node3->getGpuAddress() + node3->getContextEndOffset()) {
            notExpectedAddressInPipeControl = false;
            break;
        }
        itor = find<typename FamilyType::PIPE_CONTROL *>(++itor, cmdList.end());
    }
    EXPECT_TRUE(notExpectedAddressInPipeControl);
}

HWTEST_F(CommandQueueHwTest, GivenEventWithRootDeviceSyncNodesWhenProcessDispatchForMarkerWithTimestampPacketCalledThenEndNodeWillBeSignaledByPipeControl) {
    ...
    myCmdQ->timestampPacketContainer = std::make_unique<TimestampPacketContainer>();
    myCmdQ->timestampPacketContainer->add(pCmdQ->getContextPtr()->getMultiRootDeviceTimestampPacketAllocator()->getTag());
    ...
    NEO::LinearStream linearStream(streamBuffer, sizeof(streamBuffer));
    CsrDependencies deps = {};
    myCmdQ->processDispatchForMarkerWithTimestampPacket(*myCmdQ.get(), &linearStream, eventsRequest, deps);
...
HWTEST_F(CommandQueueHwTest, GivenEventWithEmptyRootDeviceSyncNodesContainerWhenProcessDispatchForMarkerWithTimestampPacketCalledThenSyncPipeControlIsNotProgrammed) {
...
HWTEST_F(CommandQueueHwTest, GivenEventRequestWithoutOutEventWhenProcessDispatchForMarkerWithTimestampPacketCalledThenSyncPipeControlIsNotProgrammed) {
