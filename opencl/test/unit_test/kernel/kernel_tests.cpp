...
HWTEST_F(KernelResidencyTest, givenKernelWhenItUsesIndirectUnifiedMemoryDeviceAllocationThenTheyAreMadeResident) {
    ...
    auto properties = UnifiedMemoryProperties(InternalMemoryType::deviceUnifiedMemory, 1, mockKernel.mockContext->getRootDeviceIndices(), mockKernel.mockContext->getDeviceBitfields());
...
HWTEST_F(KernelResidencyTest, givenKernelUsingIndirectHostMemoryWhenMakeResidentIsCalledThenAllAllocationsAreMadeResident) {
    ...
    auto hostProperties = UnifiedMemoryProperties(InternalMemoryType::hostUnifiedMemory, 1, mockKernel.mockContext->getRootDeviceIndices(), mockKernel.mockContext->getDeviceBitfields());
...
HWTEST_F(KernelResidencyTest, givenNonSharedUnifiedMemoryRequiredMemSyncWhenMakeResidentIsCalledThenAllocationIsNotDecommitted) {
    ...
    auto svmAllocationsManager = mockKernel.mockContext->getSVMAllocsManager();
    auto hostProperties = UnifiedMemoryProperties(InternalMemoryType::hostUnifiedMemory, 1, mockKernel.mockContext->getRootDeviceIndices(), mockKernel.mockContext->getDeviceBitfields());
    auto unifiedMemoryAllocation = svmAllocationsManager->createHostUnifiedMemoryAllocation(4096u, hostProperties);
    auto unifiedMemoryGraphicsAllocation = svmAllocationsManager->getSVMAlloc(unifiedMemoryAllocation);
    mockPageFaultManager->insertAllocation(unifiedMemoryAllocation, 4096u, svmAllocationsManager, mockKernel.mockContext->getSpecialQueue(pDevice->getRootDeviceIndex()), {});
...
HWTEST_F(KernelResidencyTest, givenKernelWhenMakeResidentIsCalledThenIndirectAllocationsArePacked) {
    ...
    auto deviceProperties = UnifiedMemoryProperties(InternalMemoryType::deviceUnifiedMemory, 1, ctx.getRootDeviceIndices(), ctx.getDeviceBitfields());
    deviceProperties.device = pDevice;
    auto unifiedMemoryAllocation = svmAllocationsManager->createUnifiedMemoryAllocation(4096u, deviceProperties);
    auto unifiedMemoryGraphicsAllocation = svmAllocationsManager->getSVMAlloc(unifiedMemoryAllocation);
...
HWTEST_F(KernelResidencyTest, givenKernelWhenMakeResidentIsCalledAndPackingIsDisabledThenIndirectAllocationsAreNotPacked) {
    ...
    kernel->makeResident(csr);
    auto residencyTaskCount = graphicsAllocation->getResidencyTaskCount(csr.getOsContext().getContextId());
    ...
    kernel->makeResident(csr);
    EXPECT_EQ(residencyTaskCount, graphicsAllocation->getResidencyTaskCount(csr.getOsContext().getContextId()));
...
using ThreadArbitrationPolicyKernelTest = ::testing::Test;

TEST_F(ThreadArbitrationPolicyKernelTest, givenThreadArbitrationPolicyAndIFPRequiredWhenKernelIsInitializedThenThreadArbitrationPolicyIsSetToRoundRobinPolicy) {
    for (auto policy : threadArbitrationPolicies) {
        struct MockKernelWithTAP : public MockKernelWithInternals {
            MockKernelWithTAP(Context &ctx, bool ifpRequired, ThreadArbitrationPolicy tap) : MockKernelWithInternals(ctx, MockKernelWithInternalsConfig{.requiresSubgroupIndependentForwardProgress = ifpRequired, .threadArbitrationPolicy = tap}) {
            }
        };
        UltClDeviceFactory deviceFactory{1, 0};
        MockContext mockCtx(deviceFactory.rootDevices[0]);
        MockKernelWithTAP mockKernelWithTAP{mockCtx, true, policy};
...
TEST_F(ThreadArbitrationPolicyKernelTest, givenThreadArbitrationPolicyAndIFPNotRequiredWhenKernelIsInitializedThenThreadArbitrationPolicyIsSetCorrectly) {
    for (auto policy : threadArbitrationPolicies) {
        struct MockKernelWithTAP : public MockKernelWithInternals {
            MockKernelWithTAP(Context &ctx, bool ifpRequired, ThreadArbitrationPolicy tap) : MockKernelWithInternals(ctx, MockKernelWithInternalsConfig{.requiresSubgroupIndependentForwardProgress = ifpRequired, .threadArbitrationPolicy = tap}) {
            }
        ...
        UltClDeviceFactory deviceFactory{1, 0};
        MockContext mockCtx(deviceFactory.rootDevices[0]);
        MockKernelWithTAP mockKernelWithTAP{mockCtx, false, policy};

        auto &mockKernel = *mockKernelWithTAP.mockKernel;
        EXPECT_EQ(policy, mockKernel.getDescriptor().kernelAttributes.threadArbitrationPolicy);
    }
}
