...
std::string LinuxInfoLogImp::getTracefsInstancesDirPath() {
    ...
    for (const auto &tracingDir : tracefsPaths) {
        ...
        if (SysmanSysCallsWrapper::access(instancesDir, F_OK, errorNum) == 0) {
            return instancesDir;
        }
    }
    return "";
...
ze_result_t LinuxInfoLogImp::claimInstanceOwnership(std::string_view instanceName, int &ownershipFd) {
    std::string instancesDir = getTracefsInstancesDirPath();
    if (instancesDir.empty()) {
        PRINT_STRING(NEO::debugManager.flags.PrintDebugMessages.get(), stderr, "Error@ %s(): Failed to locate the tracefs instances directory, returning error: 0x%x\n", NEO_FUNCTION_NAME, ZE_RESULT_ERROR_NOT_AVAILABLE);
        return ZE_RESULT_ERROR_NOT_AVAILABLE;
    }

    int errorNum = 0;
    std::string instanceDir = instancesDir + "/";
    ...
    if (fd < 0) {
        ...
        PRINT_STRING(NEO::debugManager.flags.PrintDebugMessages.get(), stderr, "Error@ %s(): Failed to open the directory of tracefs instance '%.*s', returning error: 0x%x\n",
                     NEO_FUNCTION_NAME, static_cast<int>(instanceName.length()), instanceName.data(), result);
        return result;
    }
    ...
    if (SysmanSysCallsWrapper::flock(fd, LOCK_EX | LOCK_NB, errorNum) != 0) {
        PRINT_STRING(NEO::debugManager.flags.PrintDebugMessages.get(), stderr, "Error@ %s(): Tracefs instance '%.*s' is already owned by another sysman collection instance, errno: %d, returning error: 0x%x\n",
                     NEO_FUNCTION_NAME, static_cast<int>(instanceName.length()), instanceName.data(), errorNum, ZE_RESULT_ERROR_HANDLE_OBJECT_IN_USE);
...
ze_result_t LinuxInfoLogImp::createInstance(const char *pInstanceName,
    ...
    if (pInstanceName != nullptr) {
        ...
        if (pTraceFsInstance == nullptr) {
            PRINT_STRING(NEO::debugManager.flags.PrintDebugMessages.get(), stderr, "Error@ %s(): Failed to create tracefs instance '%s', returning error: 0x%x\n", NEO_FUNCTION_NAME, pInstanceName, ZE_RESULT_ERROR_UNKNOWN);
            return ZE_RESULT_ERROR_UNKNOWN;
        }

        preExisting = !pTraceFsApi->traceFsInstanceIsNew(pTraceFsInstance);
