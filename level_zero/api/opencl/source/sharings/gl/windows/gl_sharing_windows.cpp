...
#include "level_zero/api/opencl/source/helpers/gl_helper.h"
...
GlArbSyncEvent *GLSharingFunctionsWindows::getOrCreateGlArbSyncEvent(Event &baseEvent) {
    if (baseEvent.getGlArbSyncEvent()) {
        return baseEvent.getGlArbSyncEvent();
    }

    return GlArbSyncEvent::create(baseEvent);
}
