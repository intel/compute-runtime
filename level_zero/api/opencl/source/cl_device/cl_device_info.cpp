...
#include "level_zero/api/opencl/source/cl_device/cl_device_info_map.h"
...
cl_int ClDevice::getDeviceInfo(cl_device_info paramName,
    ...
    case CL_DEVICE_IP_VERSION_INTEL: {
        ...
        if (0 == hwIpVersion) {
            auto &compilerProductHelper = getDevice().getCompilerProductHelper();
    ...
    case CL_DEVICE_NUM_SLICES_INTEL:
        param.uint = static_cast<cl_uint>(getHardwareInfo().gtSystemInfo.SliceCount * std::max(getDevice().getNumGenericSubDevices(), 1u));
        src = &param.uint;
        retSize = srcSize = sizeof(cl_uint);
        break;
    ...
    case CL_DEVICE_UUID_KHR:
        if (getDevice().getUuid(uuid) == false) {
            getDevice().generateUuid(uuid);
        }
        src = uuid.data();
    ...
    case CL_DEVICE_LUID_VALID_KHR:
        param.boolean = getDevice().verifyAdapterLuid();
        src = &param.boolean;
    ...
    case CL_DEVICE_NODE_MASK_KHR:
        ...
        getDevice().getAdapterMask(param.uint);
        src = &param.uint;
        retSize = srcSize = sizeof(cl_uint);
        break;
    ...
    case CL_L0_DEVICE_HANDLE: {
        ...
        src = &param.bitfield;
        srcSize = retSize = sizeof(ze_device_handle_t);
    } break;
    default:
