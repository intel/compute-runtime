...
void Device::bcsSplitReleaseResources() {
    bcsSplit->releaseResources();
}

void Device::setDriverHandle(DriverHandle *driverHandle) {
    this->driverHandle = driverHandle;
}

ze_result_t Device::getStatus() {
...
ze_result_t Device::createInternalCommandList(const ze_command_list_desc_t *desc,
                                              ze_command_list_handle_t *commandList) {
...
void Device::adjustCommandQueueDesc(uint32_t &ordinal, uint32_t &index) {
    ...
    if (nodeOrdinal != -1) {
        ...
        auto engineType = NEO::EngineHelpers::remapEngineTypeToHwSpecific(static_cast<aub_stream::EngineType>(nodeOrdinal), neoDevice->getRootDeviceEnvironment());

        auto engineGroupType = gfxCoreHelper.getEngineGroupType(engineType, NEO::EngineUsage::regular, hwInfo);
...
void Device::populateSubDeviceCopyEngineGroups() {
...
uint32_t Device::getCopyQueueGroupsFromSubDevice(uint32_t numberOfSubDeviceCopyEngineGroupsRequested,
...
uint32_t Device::getCopyEngineOrdinal() const {
    auto retVal = tryGetCopyEngineOrdinal();
    UNRECOVERABLE_IF(!retVal.has_value());
    return retVal.value();
}

std::optional<uint32_t> Device::tryGetCopyEngineOrdinal() const {
...
ze_result_t Device::getCommandQueueGroupProperties(uint32_t *pCount,
                                                   ze_command_queue_group_properties_t *pCommandQueueGroupProperties) {
...
ze_result_t Device::createModule(const ze_module_desc_t *desc, ze_module_handle_t *module,
...
ze_result_t Device::getComputeProperties(ze_device_compute_properties_t *pComputeProperties) {
...
NEO::EngineGroupType Device::getInternalEngineGroupType() {
    ...
    if (this->fabricVertex != nullptr && peerDevice->fabricVertex != nullptr) {
        ...
        if (directEdgeCount > 0) {
            ...
            for (const auto &edge : edges) {
                ...
                if (fabricEdgeModelSupportsBandwidthAndLatency(edgeProperties.model)) {
...
ze_result_t Device::getP2PProperties(ze_device_handle_t hPeerDevice,
    ...
    Device *peerDevice = Device::fromHandle(hPeerDevice);
...
ze_result_t Device::getRootDevice(ze_device_handle_t *phRootDevice) {
...
ze_result_t Device::getMemoryAccessProperties(ze_device_memory_access_properties_t *pMemAccessProperties) {
    auto &hwInfo = this->getHwInfo();
    auto &productHelper = this->getProductHelper();
...
ze_result_t Device::getKernelProperties(ze_device_module_properties_t *pKernelProperties) {
...
ze_result_t Device::getVectorWidthPropertiesExt(uint32_t *pCount, ze_device_vector_width_properties_ext_t *pVectorWidthProperties) {
    if (*pCount == 0) {
        *pCount = 1;
        return ZE_RESULT_SUCCESS;
    }
...
ze_result_t Device::getGlobalTimestampsUsingSubmission(uint64_t *hostTimestamp, uint64_t *deviceTimestamp) {
...
ze_result_t Device::getGlobalTimestampsUsingOsInterface(uint64_t *hostTimestamp, uint64_t *deviceTimestamp) {
...
ze_result_t Device::getCacheProperties(uint32_t *pCount, ze_device_cache_properties_t *pCacheProperties) {
    if (*pCount == 0) {
        *pCount = 1;
        return ZE_RESULT_SUCCESS;
    }
...
ze_result_t Device::reserveCache(size_t cacheLevel, size_t cacheReservationSize) {
    auto osInterface = neoDevice->getRootDeviceEnvironment().osInterface.get();
...
ze_result_t Device::setCacheAdvice(void *ptr, size_t regionSize, ze_cache_ext_region_t cacheRegion) {
    auto osInterface = neoDevice->getRootDeviceEnvironment().osInterface.get();
    if (!osInterface || osInterface->getDriverModel()->getDriverModelType() != NEO::DriverModelType::drm) {
        return ZE_RESULT_ERROR_UNSUPPORTED_FEATURE;
    }
...
ze_result_t Device::imageGetProperties(const ze_image_desc_t *desc,
...
ze_result_t Device::getDebugProperties(zet_device_debug_properties_t *pDebugProperties) {
...
ze_result_t Device::systemBarrier() { return ZE_RESULT_ERROR_UNSUPPORTED_FEATURE; }

ze_result_t Device::activateMetricGroupsDeferred(uint32_t count,
...
const NEO::GfxCoreHelper &Device::getGfxCoreHelper() {
    return this->neoDevice->getGfxCoreHelper();
}

const L0GfxCoreHelper &Device::getL0GfxCoreHelper() {
    return this->neoDevice->getRootDeviceEnvironment().getHelper<L0GfxCoreHelper>();
}

const NEO::ProductHelper &Device::getProductHelper() {
    return this->neoDevice->getProductHelper();
}

const NEO::CompilerProductHelper &Device::getCompilerProductHelper() {
    return this->neoDevice->getCompilerProductHelper();
}

NEO::OSInterface *Device::getOsInterface() { return neoDevice->getRootDeviceEnvironment().osInterface.get(); }

uint32_t Device::getPlatformInfo() const {
    const auto &hardwareInfo = neoDevice->getHardwareInfo();
    return hardwareInfo.platform.eRenderCoreFamily;
}

MetricDeviceContext &Device::getMetricDeviceContext() { return *metricContext; }

void Device::activateMetricGroups() {
...
uint32_t Device::getMaxNumHwThreads() const { return maxNumHwThreads; }

const NEO::HardwareInfo &Device::getHwInfo() const { return neoDevice->getHardwareInfo(); }
...
Device *Device::create(DriverHandle *driverHandle, NEO::Device *neoDevice, bool isSubDevice, ze_result_t *returnValue, L0::Device *deviceL0) {
    ...
    for (auto &neoSubDevice : neoDevice->getSubDevices()) {
        ...
        auto subDeviceImp = Device::fromHandle(subDevice);

        subDeviceImp->setDebugSurface(debugSurface);
        subDeviceImp->rootDevice = device;
        device->subDevices.push_back(subDeviceImp);
    }
...
NEO::PreemptionMode Device::getDevicePreemptionMode() const {
    return neoDevice->getPreemptionMode();
}

const NEO::DeviceInfo &Device::getDeviceInfo() const {
    return neoDevice->getDeviceInfo();
}

NEO::GraphicsAllocation *Device::allocateManagedMemoryFromHostPtr(void *buffer, size_t size, struct CommandList *commandList) {
...
NEO::GraphicsAllocation *Device::allocateMemoryFromHostPtr(const void *buffer, size_t size, bool hostCopyAllowed) {
...
NEO::GraphicsAllocation *Device::obtainReusableAllocation(size_t requiredSize, NEO::AllocationType type) {
...
void Device::storeReusableAllocation(NEO::GraphicsAllocation &alloc) {
    allocationsForReuse->pushFrontOne(alloc);
}

bool Device::isQueueGroupOrdinalValid(uint32_t ordinal) {
    auto &engineGroups = getActiveDevice()->getRegularEngineGroups();
    uint32_t numEngineGroups = static_cast<uint32_t>(engineGroups.size());
    uint32_t numSubDeviceCopyEngineGroups = static_cast<uint32_t>(this->subDeviceCopyEngineGroups.size());
...
ze_result_t Device::getCsrForOrdinalAndIndex(NEO::CommandStreamReceiver **csr, uint32_t ordinal, uint32_t index, ze_command_queue_priority_t priority, std::optional<int> priorityLevel, bool allocateInterrupt, uint8_t powerHint) {
    ...
    if (ordinal < numEngineGroups) {
        ...
        *csr = this->subDeviceCopyEngineGroups[subDeviceOrdinal].engines[index].commandStreamReceiver;
        selectedDevice = this->subDevices[subDeviceOrdinal];
    }
...
ze_result_t Device::getCsrForLowPriority(NEO::CommandStreamReceiver **csr, bool copyOnly) {
...
DebugSession *Device::getDebugSession(const zet_debug_config_t &config) {
    return debugSession.get();
}

DebugSession *Device::createDebugSession(const zet_debug_config_t &config, ze_result_t &result, bool isRootAttach) {
    if (!this->isSubdevice) {
        ...
        result = ZE_RESULT_SUCCESS;
        auto rootL0Device = getNEODevice()->getRootDevice()->getSpecializedDevice<Device>();
...
void Device::removeDebugSession() {
    debugSession.release();
}
void Device::setDebugSession(DebugSession *session) {
    debugSession.reset(session);
}
bool Device::toPhysicalSliceId(const NEO::TopologyMap &topologyMap, uint32_t &slice, uint32_t &subslice, uint32_t &deviceIndex) {
...
bool Device::toApiSliceId(const NEO::TopologyMap &topologyMap, uint32_t &slice, uint32_t &subslice, uint32_t deviceIndex) {
    auto deviceBitfield = neoDevice->getDeviceBitfield();
...
NEO::Device *Device::getActiveDevice() const {
...
uint32_t Device::getPhysicalSubDeviceId() {
...
NEO::EngineGroupType Device::getEngineGroupTypeForOrdinal(uint32_t ordinal) const {
    auto &engineGroups = getActiveDevice()->getRegularEngineGroups();
    uint32_t numEngineGroups = static_cast<uint32_t>(engineGroups.size());
...
uint32_t Device::getEventMaxPacketCount() const {
...
Device::CmdListCreateFunPtrT Device::getCmdListCreateFunc(const ze_base_desc_t *desc) {
    if (desc) {
...
void Device::getIntelXeDeviceProperties(ze_base_properties_t *extendedProperties) const {
...
void Device::setDebugSurface(NEO::GraphicsAllocation *debugSurface) {
    this->getNEODevice()->setDebugSurface(debugSurface);
}

NEO::GraphicsAllocation *Device::getDebugSurface() const {
