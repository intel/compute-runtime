...
TYPED_TEST(KernelArgImmediateTest, GivenArgSizeLargerThanRequiredWhenSettingArgThenInvalidArgSizeIsReturned) {
    TypeParam val[2] = {(TypeParam)0xaaaaaaaaULL, (TypeParam)0xbbbbbbbbULL};

    auto retVal = this->pMultiDeviceKernel->setArg(0, sizeof(TypeParam) + 1, &val[0]);
    EXPECT_EQ(CL_INVALID_ARG_SIZE, retVal);
}
