...
struct BlitXe3pHpgCoreTests : public ::testing::Test {
    void SetUp() override {
        if (is32bit) {
            GTEST_SKIP();
        }
        ...
        hwInfo.capabilityTable.blitterOperationsSupported = true;
        {
            ...
            defaultCopyEngine = productHelper->getDefaultCopyEngine();
            auto defaultBcsIndex = EngineHelpers::getBcsIndex(defaultCopyEngine);
            if (0u != defaultBcsIndex) {
                hwInfo.featureTable.ftrBcsInfo.set(defaultBcsIndex, true);
            }
        }
    ...
    aub_stream::EngineType defaultCopyEngine;
};

XE3P_CORETEST_F(BlitXe3pHpgCoreTests, givenBufferWhenProgrammingBltCommandThenSetMocs) {
    using MEM_COPY = typename FamilyType::MEM_COPY;

    auto &bcsEngine = clDevice->getEngine(defaultCopyEngine, EngineUsage::regular);
...
XE3P_CORETEST_F(BlitXe3pHpgCoreTests, givenBufferWhenProgrammingBltCommandThenSetMocsToValueOfDebugKey) {
...
XE3P_CORETEST_F(BlitXe3pHpgCoreTests, given2dBlitCommandWhenDispatchingThenSetValidSurfaceType) {
    using MEM_COPY = typename FamilyType::MEM_COPY;
