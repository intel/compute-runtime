...
ze_result_t GlobalOperationsImp::deviceGetProperties(zes_device_properties_t *pProperties) {
    ...
    while (pNext) {
        ...
        if (pNext->stype == ZES_STRUCTURE_TYPE_DEVICE_EXT_PROPERTIES) {
        ...
        } else if (pNext->stype == ZES_STRUCTURE_TYPE_OEM_SERIAL_ID_EXT_PROPERTIES) {
            auto oemSerialIdProperties = reinterpret_cast<zes_oem_serial_id_ext_properties_t *>(pNext);
            memset(oemSerialIdProperties->oemSerialId, 0, ZES_OEM_SERIAL_ID_SIZE);
            oemSerialIdProperties->length = 0;
            ...
            if (pOsGlobalOperations->getOemSerialNumber(oemSerialNumber, serialNumberLen)) {
                ...
                size_t maxLen = std::min({static_cast<size_t>(IGSC_MAX_OEM_SN_LENGTH),
                                          ...
                                          static_cast<size_t>(ZES_OEM_SERIAL_ID_SIZE - 1)});
                size_t actualLen = 0;
                for (size_t i = 0; i < maxLen; i++) {
                    if (oemSerialNumber[i] == 0 || !isprint(oemSerialNumber[i])) {
                        break;
                    }
                    oemSerialIdProperties->oemSerialId[actualLen++] = static_cast<char>(oemSerialNumber[i]);
                }
                oemSerialIdProperties->oemSerialId[actualLen] = '\0';
                oemSerialIdProperties->length = static_cast<uint16_t>(actualLen);
            } else {
...
ze_result_t GlobalOperationsImp::getDeviceHealthStatus(zes_device_health_status_ext_t *pHealth) {
    initGlobalOperations();
    return pOsGlobalOperations->getDeviceHealthStatus(pHealth);
}

ze_result_t GlobalOperationsImp::setDeviceHealthStatus(zes_device_health_status_ext_t health) {
    initGlobalOperations();
