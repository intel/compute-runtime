...
static const std::map<zes_ras_error_category_exp_t, std::vector<std::string>> categoryToListOfEventsUncorrectable = {
    ...
    {ZES_RAS_ERROR_CATEGORY_EXP_NON_COMPUTE_ERRORS,
     {"soc-fatal-psf-0", "soc-fatal-psf-1", "soc-fatal-psf-2", "soc-fatal-psf-csc-0",
      ...
      "gsc-nonfatal-ucode-parity", "gsc-nonfatal-mia-int", "gsc-nonfatal-wdg-timeout", "soc-fatal-iosf-pciaer",
      "soc-fatal-iosf-pcierr", "soc-fatal-pciaer", "soc-fatal-pcierr", "soc-fatal-serr-spi", "soc-fatal-serr-srcs",
      "soc-fatal-ur-response", "soc-fatal-ur", "soc-fatal-hbm-mca", "soc-fatal-hbm-punit-mca"}},
...
ze_result_t PmuRasUtil::rasGetStateExp2(const uint32_t count, const zes_ras_error_category_exp_t *pCategories, zes_intel_ras_state_exp2_t *pStates) {
    initRasErrors(false);
    if (groupFd < 0) {
        return ZE_RESULT_ERROR_DEPENDENCY_UNAVAILABLE;
    }
    ...
    for (auto const &catEventCount : errorCategoryToEventCount) {
        ...
        categoryToCounter[catEventCount.first] = errorCount + absoluteErrorCount[catEventCount.first];
