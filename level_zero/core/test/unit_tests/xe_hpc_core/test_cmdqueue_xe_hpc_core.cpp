...
HWTEST2_F(CommandQueueCommandsXeHpc, givenSplitBcsCopyAndImmediateCommandListWhenAppendingMemoryCopyFromHostToHostThenDoNotSplit, IsXeHpcCore) {
    ...
    auto result = commandList0->appendMemoryCopy(dstPtr, srcPtr, size, nullptr, 0, nullptr, copyParams);
    ASSERT_EQ(ZE_RESULT_SUCCESS, result);
    EXPECT_EQ(static_cast<WhiteBox<CommandList> *>(static_cast<Device *>(testL0Device.get())->bcsSplit->cmdLists[0])->cmdQImmediate->getTaskCount(), 1u);
    EXPECT_EQ(static_cast<WhiteBox<CommandList> *>(static_cast<Device *>(testL0Device.get())->bcsSplit->cmdLists[1])->cmdQImmediate->getTaskCount(), 1u);
    EXPECT_EQ(static_cast<WhiteBox<CommandList> *>(static_cast<Device *>(testL0Device.get())->bcsSplit->cmdLists[2])->cmdQImmediate->getTaskCount(), 1u);
    EXPECT_EQ(static_cast<WhiteBox<CommandList> *>(static_cast<Device *>(testL0Device.get())->bcsSplit->cmdLists[3])->cmdQImmediate->getTaskCount(), 1u);
...
HWTEST2_F(CommandQueueCommandsXeHpc, givenFlushTaskSubmissionEnabledAndSplitBcsCopyAndImmediateCommandListWhenAppendingMemoryCopyThenSuccessIsReturned, IsXeHpcCore) {
    ...
    EXPECT_EQ(static_cast<WhiteBox<CommandList> *>(static_cast<Device *>(testL0Device.get())->bcsSplit->cmdLists[2])->cmdQImmediate->getTaskCount(), 1u);
    EXPECT_EQ(static_cast<WhiteBox<CommandList> *>(static_cast<Device *>(testL0Device.get())->bcsSplit->cmdLists[3])->cmdQImmediate->getTaskCount(), 1u);
    EXPECT_EQ(static_cast<WhiteBox<CommandList> *>(static_cast<Device *>(testL0Device.get())->bcsSplit->cmdLists[0])->getCsr(false)->peekTaskCount(), 0u);
    EXPECT_EQ(static_cast<WhiteBox<CommandList> *>(static_cast<Device *>(testL0Device.get())->bcsSplit->cmdLists[1])->getCsr(false)->peekTaskCount(), 0u);
    EXPECT_EQ(static_cast<WhiteBox<CommandList> *>(static_cast<Device *>(testL0Device.get())->bcsSplit->cmdLists[2])->getCsr(false)->peekTaskCount(), 1u);
    EXPECT_EQ(static_cast<WhiteBox<CommandList> *>(static_cast<Device *>(testL0Device.get())->bcsSplit->cmdLists[3])->getCsr(false)->peekTaskCount(), 1u);
...
HWTEST2_F(CommandQueueCommandsXeHpc, givenSplitBcsCopyAndImmediateCommandListWhenAppendingMemoryCopyAfterBarrierThenSuccessIsReturnedAndMiSemaphoresProgrammed, IsXeHpcCore) {
    ...
    *whiteBoxCmdList->getCsr(false)->getBarrierCountTagAddress() = 0u;
    ...
    for (uint32_t i = 0u; i < static_cast<Device *>(testL0Device.get())->bcsSplit->cmdLists.size() * 2; ++i) {
...
HWTEST2_F(CommandQueueCommandsXeHpc, givenSplitBcsCopyAndImmediateCommandListWhenAllocateNewEventsForSplitThenEventsAreManagedProperly, IsXeHpcCore) {
    ...
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.pools.size(), 0u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.marker.size(), 0u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.subcopy.size(), 0u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.barrier.size(), 0u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.createdFromLatestPool, 0u);

    static_cast<Device *>(testL0Device.get())->bcsSplit->events.createFromPool(Context::fromHandle(commandList0->getCmdListContext()), 12);

    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.pools.size(), 1u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.marker.size(), 1u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.subcopy.size(), 4u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.barrier.size(), 1u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.createdFromLatestPool, 6u);

    static_cast<Device *>(testL0Device.get())->bcsSplit->events.createFromPool(Context::fromHandle(commandList0->getCmdListContext()), 12);

    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.pools.size(), 1u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.marker.size(), 2u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.subcopy.size(), 8u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.barrier.size(), 2u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.createdFromLatestPool, 12u);

    static_cast<Device *>(testL0Device.get())->bcsSplit->events.createFromPool(Context::fromHandle(commandList0->getCmdListContext()), 12);

    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.pools.size(), 2u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.marker.size(), 3u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.subcopy.size(), 12u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.barrier.size(), 3u);
    EXPECT_EQ(static_cast<Device *>(testL0Device.get())->bcsSplit->events.createdFromLatestPool, 6u);
}
...
HWTEST2_F(CommandQueueCommandsXeHpc, givenSplitBcsCopyAndImmediateCommandListWhenObtainEventsForSplitThenReuseEventsIfMarkerIsSignaled, IsXeHpcCore) {
    ...
    auto ret = static_cast<Device *>(testL0Device.get())->bcsSplit->events.obtainForSplit(Context::fromHandle(commandList0->getCmdListContext()), 12);
    ...
    static_cast<Device *>(testL0Device.get())->bcsSplit->events.marker[1]->hostSignal(false);

    ret = static_cast<Device *>(testL0Device.get())->bcsSplit->events.obtainForSplit(Context::fromHandle(commandList0->getCmdListContext()), 12);
    ...
    auto memoryManager = reinterpret_cast<MockMemoryManager *>(static_cast<Device *>(testL0Device.get())->bcsSplit->getDevice().getDriverHandle()->getMemoryManager());
