...
TEST_F(ClProgramCompileLinkTests, givenSpirvIlProgramWhenBuildProgramThenVec3TypeIsPreserved) {
    auto *mockCompiler = new L0::ult::MockCompilerInterface();
    neoDevice->getExecutionEnvironment()->rootDeviceEnvironments[neoDevice->getRootDeviceIndex()]->compilerInterface.reset(mockCompiler);
    ...
    // Build fails (mock emits no binary) but the compiler still captured the options.
    clBuildProgram(program, 1, &clDeviceId, nullptr, nullptr, nullptr);
    EXPECT_TRUE(CompilerOptions::contains(mockCompiler->inputInternalOptions, CompilerOptions::preserveVec3Type))
        << mockCompiler->inputInternalOptions;

    clReleaseProgram(program);
    clReleaseContext(clContext);
}

TEST_F(ClProgramCompileLinkTests, givenSpirvIlProgramsWhenLinkProgramThenVec3TypeIsPreserved) {
    auto *mockCompiler = new L0::ult::MockCompilerInterface();
    neoDevice->getExecutionEnvironment()->rootDeviceEnvironments[neoDevice->getRootDeviceIndex()]->compilerInterface.reset(mockCompiler);
...
TEST_F(ClProgramCompileLinkTests, givenSourceProgramWhenBuildProgramThenVec3TypeIsPreserved) {
    auto *mockCompiler = new L0::ult::MockCompilerInterface();
    neoDevice->getExecutionEnvironment()->rootDeviceEnvironments[neoDevice->getRootDeviceIndex()]->compilerInterface.reset(mockCompiler);
