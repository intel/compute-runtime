...
struct MockProfilingEvent : public Event {
    ...
    ze_result_t queryKernelTimestamp(ze_kernel_timestamp_result_t &result) override {
        ...
        // thread owns it, so this distinguishes "getProfilingInfo holds ownership" from "it does not".
...
TEST_F(EventProfilingLockMtTests, givenProfilingQueryWhenResolvingTimestampThenEventOwnershipIsNotHeld) {
    auto event = std::make_unique<MockProfilingEvent>(CL_COMMAND_MARKER, commandQueue);

    // The derivation is the only region that needs ownership; skipping it leaves exactly the
    // region that may go on to block on the GPU, which must run unlocked.
    event->dataCalculated = true;
...
struct MockDerivingEvent : public Event {
    using Event::dataCalculated;
    using Event::Event;
...
TEST_F(EventProfilingLockMtTests, givenConcurrentProfilingQueriesWhenDerivingDataThenAnchorsAreRebasedOnce) {
    auto event = std::make_unique<MockDerivingEvent>(CL_COMMAND_NDRANGE_KERNEL, commandQueue);
