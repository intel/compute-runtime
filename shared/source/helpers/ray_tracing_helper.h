...
class RayTracingHelper : public NonCopyableAndNonMovableClass {
  public:
    ...
    static size_t getRTStackSizePerTile(const HardwareInfo &hwInfo, uint32_t tiles, uint32_t maxBvhLevel, uint32_t extraBytesLocal, uint32_t extraBytesGlobal) {
        return static_cast<size_t>(alignUp(getStackSizePerRay(maxBvhLevel, extraBytesLocal) * (getNumRtStacks(hwInfo)) + extraBytesGlobal, MemoryConstants::cacheLineSize));
    }
    ...
    static uint32_t getNumRtStacksPerDss(const HardwareInfo &hwInfo) {
        if (hwInfo.caps.numRtStacksPerDssFixedValue) {
