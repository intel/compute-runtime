...
ze_result_t LinuxSchedulerImp::getPreemptTimeout(uint64_t &timeout, ze_bool_t getDefault) {
    ...
    for (const auto &engineName : listOfEngines) {
        ...
        if (result == ZE_RESULT_SUCCESS) {
            ...
            PRINT_STRING(NEO::debugManager.flags.PrintDebugMessages.get(), stderr, "Error@ %s(): Failed to read preempt timeout from %s and returning error:0x%x \n", NEO_FUNCTION_NAME, path.c_str(), result);
            return result;
        }
    }
...
ze_result_t LinuxSchedulerImp::getTimesliceDuration(uint64_t &timeslice, ze_bool_t getDefault) {
    ...
    for (const auto &engineName : listOfEngines) {
        ...
        if (result == ZE_RESULT_SUCCESS) {
            ...
            PRINT_STRING(NEO::debugManager.flags.PrintDebugMessages.get(), stderr, "Error@ %s(): Failed to read timeslice duration from %s and returning error:0x%x \n", NEO_FUNCTION_NAME, path.c_str(), result);
            return result;
        }
    }
    // check if all engines of the same type have the same scheduling param values
...
ze_result_t LinuxSchedulerImp::getHeartbeatInterval(uint64_t &heartbeat, ze_bool_t getDefault) {
    ...
    for (const auto &engineName : listOfEngines) {
        ...
        if (result == ZE_RESULT_SUCCESS) {
            ...
            PRINT_STRING(NEO::debugManager.flags.PrintDebugMessages.get(), stderr, "Error@ %s(): Failed to read heartbeat interval from %s and returning error:0x%x \n", NEO_FUNCTION_NAME, path.c_str(), result);
            return result;
        }
    }
    // check if all engines of the same type have the same scheduling param values
...
ze_result_t LinuxSchedulerImp::setPreemptTimeout(uint64_t timeout) {
    ...
    for (const auto &engineName : listOfEngines) {
        ...
        if (result != ZE_RESULT_SUCCESS) {
            ...
            PRINT_STRING(NEO::debugManager.flags.PrintDebugMessages.get(), stderr, "Error@ %s(): Failed to write Preempt timeout into engineDir/%s/preemptTimeoutMilliSecs and returning error:0x%x \n", NEO_FUNCTION_NAME, engineName.c_str(), result);
            return result;
        }
    }
    return result;
...
ze_result_t LinuxSchedulerImp::setTimesliceDuration(uint64_t timeslice) {
    ...
    for (const auto &engineName : listOfEngines) {
        ...
        if (result != ZE_RESULT_SUCCESS) {
            ...
            PRINT_STRING(NEO::debugManager.flags.PrintDebugMessages.get(), stderr, "Error@ %s(): Failed to write Timeslice duration into engineDir/%s/timesliceDurationMilliSecs and returning error:0x%x \n", NEO_FUNCTION_NAME, engineName.c_str(), result);
            return result;
        }
    }
    return result;
...
ze_result_t LinuxSchedulerImp::setHeartbeatInterval(uint64_t heartbeat) {
    ...
    for (const auto &engineName : listOfEngines) {
        ...
        if (result != ZE_RESULT_SUCCESS) {
            ...
            PRINT_STRING(NEO::debugManager.flags.PrintDebugMessages.get(), stderr, "Error@ %s(): Failed to write Heartbeat interval into engineDir/%s/heartbeatIntervalMilliSecs and returning error:0x%x \n", NEO_FUNCTION_NAME, engineName.c_str(), result);
            return result;
        }
    }
    return result;
