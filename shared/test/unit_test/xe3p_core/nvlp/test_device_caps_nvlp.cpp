...
using NvlDeviceIdTest = Test<DeviceFixture>;

NVLPTEST_F(NvlDeviceIdTest, givenNvlProductWhenCheckingCapabilitiesThenReturnCorrectValues) {
    ...
    EXPECT_TRUE(NVLP::hwInfo.capabilityTable.supportsImages);

    EXPECT_EQ(1024u, pDevice->getDeviceInfo().maxWorkGroupSize);

    EXPECT_EQ(192u, pDevice->getHardwareInfo().capabilityTable.maxProgrammableSlmSize);
...
NVLPTEST_F(NvlDeviceIdTest, givenSlmSizeAndHeaplessWhenEncodingThenReturnCorrectValues) {
